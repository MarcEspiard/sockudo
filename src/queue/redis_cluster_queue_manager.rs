...
impl QueueInterface for RedisClusterQueueManager {
        ...
        JobData: DeserializeOwned + Send + 'static, // Ensure JobData can be deserialized and sent across threads
    {
        ...
        for i in 0..self.concurrency {
            ...
            let handle = tokio::spawn(async move {
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = async {
                            ...
                            info!("Redis cluster queue worker {} for queue {} received shutdown signal", i, worker_queue_name);
                            break;
                        }
                        // Process jobs
                        blpop_result = async {
                ...
                info!("Redis cluster queue worker {} for queue {} shut down", i, worker_queue_name);
            });
    ...
    async fn disconnect(&self) -> crate::error::Result<()> {
                ...
                match tokio::time::timeout(Duration::from_secs(3), handle).await {
                    Ok(join_result) => {
                        if let Err(e) = join_result {
                            if !e.is_cancelled() {
                                warn!("Redis cluster queue worker completed with error: {}", e);
                            }
                        }
                    }
                    Err(_) => {
                        warn!("Redis cluster queue worker did not complete within timeout");
                    }
                }
            }
        }

        // Clean up Redis cluster queues
        let mut conn = self.redis_connection.lock().await;
        ...
        info!("Redis cluster queue manager disconnected successfully");
        Ok(())
    }
}
